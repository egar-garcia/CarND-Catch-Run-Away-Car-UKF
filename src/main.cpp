  ...
  h.onMessage([&ukf,&target_x,&target_y](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        // Calculating the point where the capture will occur
        VectorXd target_point = ukf.GetCatchingPoint(distance_difference);
        ...
        target_y = target_point[1];
    	  ...
    	  while (heading_difference <-M_PI) heading_difference+=2.*M_PI;
        heading_difference *= 0.1; // To smooth turns
